...
public class RegistMapper {
    ...
    public Regist selectByIdNumber(String identityNumber){
        try{
            pstmt =conn.prepareStatement("select  * from regist_t where identiy_number=? and delsign=0");
            pstmt.setString(1,identityNumber);
            ResultSet rs =pstmt.executeQuery();
            while (rs.next()){
                Regist regist =new Regist();
                regist.setRegistId(rs.getInt("regist_id"));
                regist.setExamineeNumber(rs.getInt("examinee_number"));
...
public boolean insert(Regist regist){
        try{
            ...
            pstmt.setLong(1,regist.getExamineeNumber());
